...
sol: !pip install mlxtend

plt for svm_pca was taking too long, so i edited the parameters to reduce number of X_train_pca samples.
...
changed to:
X_train_pca_small = X_train[:2000]  # Use only 2000 points
...
for classificaion report:
got 'undefinemetricwarning' because label 0 has no correctly predicted samples.
